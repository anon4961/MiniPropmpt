...
function renderActiveSummary() {
    ...
    const charSetsHtml = summary.characterSets.length === 0
        ...
        : summary.characterSets.map(s =>
            `<span class="mcp-summary-tag mcp-summary-tag-on" title="${escapeHtml(s.name)}">${escapeHtml(s.name)} <small>(${s.activeSlots}/${s.totalSlots})</small></span>`
        ).join(' ');
...
<div class="mcp-preview-content">
    <div class="mcp-preview-summary">
        <div>주입될 메시지: <b>${messages.length}</b>개 (캐릭터+채팅방 합산)</div>
